...
export class EventComponent implements OnInit {
  ...
  eventForm = this.fb.group({
    event_list_comments: [''],
    total_repetitions: [],
    file_paths: [[]]  // Add this new form control
  });
  ...
  processFiles(items: FileList) {
    const paths = [];
    for (let i = 0; i < items.length; i++) {
      const item = items[i];
      if (item.webkitRelativePath) {
        const filePath = item.webkitRelativePath;
        if (this.isAllowedExtension(item.name)) {
          paths.push(filePath); // Collect file paths
    ...
    this.eventForm.patchValue({ file_paths: paths }); // Save file paths in the form control
  }
  ...
  <form [formGroup]="eventForm">
    <div class="event-list-column">
      <div class="file-note-row">
        <div class="note-row">
          <!-- Additional notes or information can be added here -->
        </div>
        <div class="form-row-item file-upload-div">
          <div class="file-upload-div-inner">
            <div>Upload event information files</div>
            <input type="file" webkitdirectory (change)="onFolderSelected($event)" multiple />
          </div>
        </div>
      </div>
      ...
      <!-- Optionally, display file paths stored in the form -->
      <div class="saved-file-paths">
        <div *ngFor="let path of eventForm.value.file_paths">
          {{ path }}
        </div>
      </div>
    </div>
