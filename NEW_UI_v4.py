# -*- coding: utf-8 -*-
"""
Created on Wed Nov  8 16:44:00 2023
...
class PDFViewer:
    ...
    def on_button_press(self, event):
        ...
        if self.rect:
            self.label.delete(self.rect)

    def on_move_press(self, event):
        if self.rect:
            self.label.delete(self.rect)
        self.rect = self.label.create_rectangle(self.start_x, self.start_y, event.x, event.y, outline='red')

    def perform_ocr(self):
        if self.current_image and self.rect:
            region = self.label.coords(self.rect)
            x0, y0, x1, y1 = int(region[0]), int(region[1]), int(region[4]), int(region[5])
            cropped_image = self.current_image.crop((x0, y0, x1, y1))

            # Perform OCR using EasyOCR on the cropped image
