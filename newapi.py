...
def extract_tube_info_to_excel(part_numbers, auth):
    ...
    for part_number in part_numbers:
        ...
        if tube_info:
            # Assuming tube_info is a list and has elements at specific positions
            data_row = [part_number,
                        tube_info[0] if len(tube_info) > 0 else '',
                        tube_info[1] if len(tube_info) > 1 else '',
                        tube_info[2] if len(tube_info) > 2 else '',
                        tube_info[3] if len(tube_info) > 3 else '']
            data_rows.append(data_row)
